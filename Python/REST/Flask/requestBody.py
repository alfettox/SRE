...
def get_description():
    ...
    <p>
        The four request methods we will use are GET, POST, PUT, and DELETE. GET requests can be used in a browser.
        The other methods can be used via CURL or another API platform, such as Postman or Thunder Client in VS Code.
    </p>
    <ul>
    ...
    <p>The endpoints below can be accessed over CURL using the correct request method. </p>
        ...
        <li>curl -X POST -H "Content-Type: application/json" -d '{"username": "Georgina"}' http://127.0.0.1:5000/new_user/ # POST a new user</a></li>
...
# get one user. Notice the id is captured from the URI
...
# Original new_user function that uses the URL for data
# @app.route("/new_user/<string:username>/", methods=['POST'])
# def new_user(username):
#     """
#         Because post requests are not sent over the browser, use this CURL command.
#         You can also use a tool such as Postman or Thunder Client in VS Code.

#         curl -X POST 'http://127.0.0.1:5000/new_user/Georgina/'
#     """
#     if username in users_database.values():
