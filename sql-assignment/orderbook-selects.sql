...
SELECT uname AS `User`, dateJoined AS `Date joined`FROM User;



/*
'admin', '2023-02-14Order 13:13:28'
...
SELECT orderId, userId, symbol, orderTime, shares, price, ABS(shares * price) AS `Absolute net order`, status
FROM `Order`
WHERE MONTH(orderTime) = 3 AND (ABS(shares * price)) > 1000;

SELECT symbol,
	ABS(shares) AS total_shares,
	ABS(shares * price) AS total_amount
FROM `Order`
...
SELECT orderid, fillid, userid, symbol, ABS(share*price) AS absolute_net_fill_amount
FROM Fill
HAVING absolute_net_fill_amount >1000
