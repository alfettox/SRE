...
SELECT orderid, userid, symbol, side, orderTime,shares,price,status, ABS(shares*price) AS absolute_order_net
FROM `Order`
...
SELECT ord.orderid, ord.symbol, ord.status, ord.shares, ord.price, filled.orderid, filled.symbol, filled.share AS "Filled shares", filled.price
FROM `Order` AS `ord`
JOIN `Fill` AS filled
ON filled.orderid = ord.orderid;
...
/*
14	SPY		filled	-75	75	365.73	27429.75
8	AAPL	filled	25	-10	140.76	3519.00
8	AAPL	filled	25	-15	140.76	3519.00
7	GS		filled	-10	10	305.63	3056.30
10	AAPL	filled	-15	15	140.76	2111.40
11 ROWS RETURNED
*/
...
JOIN UserRoles AS  ur  ON u.userid = ur.userid
JOIN `Role` AS r  ON ur.roleid = r.roleid
WHERE o.status NOT LIKE "filled"
