...
SELECT COUNT(userId) AS "number of users", SUM(userId) as "sum"
from User;
...
SELECT u.uname,u.userid, COUNT(o.userid) AS numOrders
FROM User AS u
JOIN `Order` AS o
...
SELECT u.uname AS username,
	   ROUND(SUM(ABS(o.price * o.shares))/COUNT(o.orderId),2) AS average_net_order
FROM User AS u
JOIN `Order` AS o
...
SELECT u.uname, f.symbol, SUM(f.share)
FROM `User` AS u
JOIN `Fill` AS f ON u.userId = f.userId
